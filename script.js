...
$('back-cover').addEventListener('click',()=>audio.pause());
if('serviceWorker' in navigator&&location.protocol!=='file:'){
 const offline=document.createElement('p');offline.id='offline-status';offline.style.fontSize='16px';offline.setAttribute('role','status');offline.textContent='Guardando a música para ouvir sem internet...';$('audio-status').after(offline);
 navigator.serviceWorker.register('./sw.js').then(async()=>{await navigator.serviceWorker.ready;offline.textContent='Cartinha e música guardadas para abrir sem internet neste aparelho ♡';}).catch(()=>{offline.textContent='A música toca aqui; não foi possível guardar para ouvir offline.';});
}
